...
struct GDT_Entry
gdt_entry(uint32_t base, uint16_t limit, uint8_t access, uint8_t flags)
{
...
int init_gdt()
{
    a20_enable();
    struct GDT gdt;
    gdt.null_descriptor = gdt_entry(0, 0x00000000, 0x00, 0x0);
    ...
    gdt.task_state_segment = gdt_entry(0, 0, 0x89, 0x0);
